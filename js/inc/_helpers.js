(function ($, root, undefined) {
    ...
    $.fn.mobileMenu = function (selector = 'nav#nav_mobile', side = 'left') {
        ...
        // prevent empty links closing menu
        $(selector).on('click', 'a[href]', function (event) {
            ...
            if (!href || href === '#') {
                ...
                if (parent.length) {
                    parent.find('.mm-next').trigger('click');
                    parent.find('.mm-prev').trigger('click');
     ...
     * accordion on mobile
     */
    $.fn.footerAccordion = function (breakpoint = 900) {

        var winIsSmall;

        $(document).on('click', '.footer-menu--accordion .footer-menu__heading', function (event) {
            event.preventDefault();
            console.log('click')
            $(this).closest('.footer-menu--accordion').toggleClass('footer-menu--accordion-active');
        });

        $(window).on('load resize', function () {
            winIsSmall = window.innerWidth <= breakpoint;
            $('.footer-menu').toggleClass('footer-menu--accordion', winIsSmall);
        });
